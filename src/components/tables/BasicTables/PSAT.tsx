...
if (cleanExamPeriod.length === 0) {
  ...
  // --- 4. DELETE DATA (Modified Logic: Overwrite with Empty) ---
  const handleDelete = async () => {
    // Confirmation Alert
    if (!window.confirm("Are you sure you want to delete ALL data? This will clear everything from the website.")) {
      return;
    }
    ...
    try {
      ...
      // LOGIC CHANGED: DELETE request ki jagah hum khali data ke sath POST request bhej rahe hain.
      // Isse database me purana data overwrite hokar blank ho jayega.
      const emptyPayload = {
        heroTitle: "",
        heroSubtitle: "",
        heroDescription: "",
      ...
      setForm(initialState); // UI ko bhi reset kar diya
    } catch (err) {
      console.error(err);
