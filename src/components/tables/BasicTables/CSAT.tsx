...
export default function ScatTestPrepAdmin() {
  ...
  const validateForm = () => {
    ...
    const requiredFields = [
      ...
      { key: "versionsHeading", label: "Versions Heading" },
      { key: "formatHeading", label: "Format Heading" },
      { key: "formatDescription", label: "Format Description" },
      { key: "scoringHeading", label: "Scoring Heading" },
      { key: "scoringFooter", label: "Scoring Footer" },
      { key: "tipsHeading", label: "Tips Heading" },
      { key: "registerHeading", label: "Register Heading" },
      { key: "registerSubHeading", label: "Register Sub-Heading" },
      { key: "registerAuthNote", label: "Register Auth Note" },
    ];
    ...
    // 2. Check Simple Lists (must not be empty array and no empty strings)
    if (form.versionsList.length === 0 || form.versionsList.some(i => !i.trim())) {
      toast.error("Please fill all Version items (at least one required)");
      return false;
    }
    if (form.tipsList.length === 0 || form.tipsList.some(i => !i.trim())) {
      toast.error("Please fill all Tips items (at least one required)");
      return false;
    }
    if (form.registerContactList.length === 0 || form.registerContactList.some(i => !i.trim())) {
      toast.error("Please fill all Contact items (at least one required)");
      return false;
    }
    if (form.registerGetFromList.length === 0 || form.registerGetFromList.some(i => !i.trim())) {
      toast.error("Please fill all 'Get From' items (at least one required)");
      return false;
    }

    // 3. Check Object Arrays (Format Sections & Scoring Levels)
    if (form.formatSections.length === 0 || form.formatSections.some(sec => !sec.title.trim() || !sec.description.trim())) {
      toast.error("All Format Sections must have a Title and Description");
      return false;
    }
    if (form.scoringLevels.length === 0 || form.scoringLevels.some(lvl => !lvl.title.trim() || !lvl.details.trim())) {
      toast.error("All Scoring Levels must have a Name and Details");
      return false;
    }
    ...
    <div className="bg-gray-50 p-6 flex flex-col items-center min-h-screen">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-5xl border-t-4 border-blue-600 p-8">
        ...
        <div className="mb-6 border-b pb-6">
          ...
          <label className="font-semibold text-sm">Versions Heading <ReqStar />:</label>
          <input name="versionsHeading" value={form.versionsHeading} onChange={handleChange} placeholder="Versions Heading" className="w-full mt-1 mb-2 p-2 border rounded bg-gray-50" />
          
          <label className="font-semibold text-sm">Versions List <ReqStar />:</label>
          {form.versionsList.map((pt, i) => (
            <div key={i} className="flex gap-2 mb-1">
              <input value={pt} onChange={(e)=>handleListChange(i, e.target.value, 'versionsList')} className="w-full p-2 border rounded" placeholder="e.g. Grades 2-3..." />
              <button onClick={()=>removeListItem(i, 'versionsList')} className="text-red-500 font-bold">✕</button>
            </div>
          ))}
          <button onClick={()=>addListItem('versionsList')} className="text-blue-600 text-sm mt-1">+ Add Version</button>
        </div>
        ...
        <div className="mb-6 border-b pb-6">
          ...
          <label className="block text-sm font-semibold mb-2">Sections (All fields required) <ReqStar /></label>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             {form.formatSections.map((sec, i) => (
                <div key={i} className="bg-gray-50 p-3 rounded relative border border-gray-200">
                   <button onClick={()=>removeObjectItem(i, 'formatSections')} className="absolute top-1 right-1 text-red-500 font-bold">✕</button>
                   <input value={sec.title} onChange={(e)=>handleObjectArrayChange(i, 'title', e.target.value, 'formatSections')} placeholder="Section Title" className="w-full mb-2 p-1 border rounded font-bold" />
                   <textarea value={sec.description} onChange={(e)=>handleObjectArrayChange(i, 'description', e.target.value, 'formatSections')} placeholder="Desc..." rows={3} className="w-full p-1 border rounded" />
                </div>
             ))}
        ...
        <div className="mb-6 border-b pb-6">
          ...
          <label className="block text-sm font-semibold mb-2">Levels (All fields required) <ReqStar /></label>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
             {form.scoringLevels.map((lvl, i) => (
                <div key={i} className="border p-3 rounded relative border-gray-200">
                   <button onClick={()=>removeObjectItem(i, 'scoringLevels')} className="absolute top-1 right-1 text-red-500 font-bold">✕</button>
                   <input value={lvl.title} onChange={(e)=>handleObjectArrayChange(i, 'title', e.target.value, 'scoringLevels')} placeholder="Level Name" className="w-full mb-2 p-1 border rounded font-bold" />
                   <textarea value={lvl.details} onChange={(e)=>handleObjectArrayChange(i, 'details', e.target.value, 'scoringLevels')} placeholder="Scores (Verbal/Quant)..." rows={3} className="w-full p-1 border rounded" />
                </div>
             ))}
          ...
          <label className="block text-sm font-semibold mt-3 mb-1">Footer Note <ReqStar /></label>
          <input name="scoringFooter" value={form.scoringFooter} onChange={handleChange} placeholder="Footer Note" className="w-full p-2 border rounded text-sm italic" />
        </div>
        ...
        <div className="mb-6 border-b pb-6">
          ...
          <label className="block text-sm font-semibold mb-1">Tips List <ReqStar /></label>
          {form.tipsList.map((pt, i) => (
            <div key={i} className="flex gap-2 mb-1">
              <input value={pt} onChange={(e)=>handleListChange(i, e.target.value, 'tipsList')} className="w-full p-2 border rounded" placeholder="Tip..." />
              <button onClick={()=>removeListItem(i, 'tipsList')} className="text-red-500 font-bold">✕</button>
            </div>
          ))}
          <button onClick={()=>addListItem('tipsList')} className="text-blue-600 text-sm">+ Add Tip</button>
        </div>
        ...
        <div className="mb-6">
          ...
          <label className="block text-sm font-semibold mb-1">Main Heading <ReqStar /></label>
          <input name="registerHeading" value={form.registerHeading} onChange={handleChange} placeholder="Main Heading" className="w-full mb-2 p-2 border rounded font-bold" />
          
          <label className="block text-sm font-semibold mb-1">Sub Heading <ReqStar /></label>
          <input name="registerSubHeading" value={form.registerSubHeading} onChange={handleChange} placeholder="Sub Heading (Schedule...)" className="w-full mb-3 p-2 border rounded" />
          
          <label className="text-sm font-semibold block mb-1">Contact Methods <ReqStar />:</label>
          {form.registerContactList.map((pt, i) => (
            <div key={i} className="flex gap-2 mb-1">
              <input value={pt} onChange={(e)=>handleListChange(i, e.target.value, 'registerContactList')} className="w-full p-2 border rounded" placeholder="e.g. Phone: 800..." />
              <button onClick={()=>removeListItem(i, 'registerContactList')} className="text-red-500 font-bold">✕</button>
            </div>
          ))}
          <button onClick={()=>addListItem('registerContactList')} className="text-blue-600 text-sm mb-4">+ Add Contact Method</button>

          <label className="block text-sm font-semibold mb-1">Auth Note <ReqStar /></label>
          <input name="registerAuthNote" value={form.registerAuthNote} onChange={handleChange} placeholder="Auth Number Note" className="w-full mb-3 p-2 border rounded" />
          
          <label className="text-sm font-semibold block mb-1">Get it from List <ReqStar />:</label>
          {form.registerGetFromList.map((pt, i) => (
            <div key={i} className="flex gap-2 mb-1">
              <input value={pt} onChange={(e)=>handleListChange(i, e.target.value, 'registerGetFromList')} className="w-full p-2 border rounded" placeholder="e.g. Mail packet..." />
              <button onClick={()=>removeListItem(i, 'registerGetFromList')} className="text-red-500 font-bold">✕</button>
            </div>
          ))}
          <button onClick={()=>addListItem('registerGetFromList')} className="text-blue-600 text-sm">+ Add Source</button>
        </div>
