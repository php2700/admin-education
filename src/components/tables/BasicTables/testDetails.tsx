...
export default function TestPrepDetail() {
  ...
  const handleSave = async () => {
    ...
    // 3. Check Table Data
    // Filter out rows where any cell is empty
    ...
    // Check if user left a partially filled row (Optional strictness)
    const hasIncompleteRow = form.tableData.some(
      row => (row.section || row.time || row.modules) && (!row.section || !row.time || !row.modules)
    );
    if (hasIncompleteRow) {
      return toast.warning("Please complete all fields in every table row or remove the empty row.");
    }
